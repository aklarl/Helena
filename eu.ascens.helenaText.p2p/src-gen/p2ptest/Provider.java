...
public class Provider extends Role {
	...
	protected ProcessExpression initializeRoleBehavior()
		...
		ProcessInvocation invoc82612980 = new ProcessInvocation();

		ProcessExpression providerProc = new ActionPrefix(new ReceiveMessageAction(new ReqFileMessage(req)),
				new ActionPrefix(

						new SendMessageAction(this.req,
								new SndFileMessage(new Variable<>(
										new CompAttrGetter<>("content", Integer.class).getValue(this), Integer.class))),
						Quit.getInstance()));

		invoc82612980.setInvocatedProcess(providerProc);
